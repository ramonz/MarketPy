...
class MOEXStocks(Schedule):
    ...
    def __init__(self):
        super(MOEXStocks, self).__init__([
            Session(time(10, 0, 0), time(18, 44, 59)),  # Основная торговая сессия
            Session(time(19, 0, 0), time(23, 49, 59))  # Вечерняя торговая сессия
...
class MOEXFutures(Schedule):
    ...
    def __init__(self):
        super(MOEXFutures, self).__init__([
            ...
            Session(time(14, 0, 0), time(18, 49, 59)),  # Основная торговая сессия (Вечерний расчетный период)
            Session(time(19, 0, 0), time(23, 49, 59))  # Вечерняя дополнительная торговая сессия
        ], timedelta(seconds=3))  # Задержка 3 секунды, чтобы гарантированно получить бар
